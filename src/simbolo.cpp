...
char *Simbolo::getNome() { return this->nome; }
char *Simbolo::getTipo() { return this->tipo; }

char *Simbolo::getClasse() { return this->classe; }

void Simbolo::setClasse(char classe[21]) { strncpy(this->classe, classe, 21); }

void Simbolo::setNome(char nome[21]) { strncpy(this->nome, nome, 21); }

void Simbolo::setTipo(char tipo[21]) { strncpy(this->tipo, tipo, 21); }
void Simbolo::setEscopo(char escopo[21]) { strncpy(this->scopo, escopo, 21); }

void Simbolo::print() {
  std::cout << this->getNome() << " " << this->getScopo() << " "
            << this->getClasse() << " " << this->getTipo() << std::endl;
}
...
char *Simbolo::getScopo() { return this->scopo; }
...
int existeNomeEmEscopo(std::list<Simbolo *> &simbs, char scopo[21],
                       char nome[21]) {
...
char *tipoNomeEmEscopo(std::list<Simbolo *> &simbs, char scopo[21],
                       char nome[21]) {
  std::list<Simbolo *>::iterator i;
  for (i = simbs.begin(); i != simbs.end(); ++i) {
    if (!strcmp((*i)->getNome(), nome))       // retorna 0 se é igual
      if (!strcmp((*i)->getScopo(), scopo)) { // retorna 0 se é igual
        if (!strcmp((*i)->getClasse(), "var") ||
            !strcmp((*i)->getClasse(), "param")) {
          return (*i)->getTipo();
        }
      }
  }
...
char *classeNomeEmEscopo(std::list<Simbolo *> &simbs, char scopo[21],
                         char nome[21]) {
  std::list<Simbolo *>::iterator i;
  for (i = simbs.begin(); i != simbs.end(); ++i) {
    if (!strcmp((*i)->getNome(), nome))       // retorna 0 se é igual
...
char *tipoParamFuncao(std::list<Simbolo *> &simbs, char nome_func[21],
                      int posicao) {
  std::list<Simbolo *>::iterator i;
  for (i = simbs.begin(); i != simbs.end(); ++i) {
