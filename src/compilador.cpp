...
No *const_valor(std::ifstream &fin, Token &tk, int *linha, int *col) {
  ...
  if (tk.tipo ==
      cte::IDENTIFICADOR) { // o que ele diz de sequencia alfanumerica
...
/*
void erro(char tag[20], char info[10],char desc[130],bool tipo){ //tipo 0 ->
erro sintatico | 1 semantico if(tipo==0) std::cout<< "erro em "<<tag<<", falta
"<<info<<std::endl; if(tipo==1) std::cout<<info<<"  erro semantico "<<de
                     sc<<std::endl;
}
*/
  ...
  if (tk.tipo == cte::IDENTIFICADOR) {
    ...
    if (tk.tipo == cte::IGUAL) {
      ...
      if (!existeNomeEmEscopo(smbs, escopo, iden)) {
        //                std::cout<<
        //                "---------"<<larv->filho->prox->prox->filho->filho->filho->token.info<<std::endl;
        smbs.push_back(new SimboloConst(
            iden, escopo, (char *)"const", (char *)"int",
            atol(larv->filho->prox->prox->filho->filho->filho->token.info)));
      } else {
        strcpy(desc, escopo);
...
No *tipo_dado(std::ifstream &fin, Token &tk, int *linha, int *col) {
  ...
  if (tk.tipo == cte::REAL || tk.tipo == cte::INTEIRO) {
    ...
    if (tk.tipo == cte::ACOLCH) {
      larv->filho->prox = insereLista(larv->filho->prox, tk);
      tk = getToken(fin, linha,
                    col); // acabei de colocar pra tirar o problema de numero
      larv->filho->prox->prox = numero(fin, tk, linha, col);
      if (tk.tipo == cte::FCOLCH) {
        larv->filho->prox->prox->prox =
            insereLista(larv->filho->prox->prox->prox, tk);
        tk = getToken(fin, linha, col);
        if (tk.tipo == cte::DE) {
          larv->filho->prox->prox->prox->prox =
              insereLista(larv->filho->prox->prox->prox->prox, tk);
          tk = getToken(fin, linha, col);
          larv->filho->prox->prox->prox->prox->prox =
              tipo_dado(fin, tk, linha, col);
          return larv;
        }
      }
    }
...
No *lista_id(std::ifstream &fin, Token &tk, int *linha, int *col, int funcao,
             int &qnt_parms) {
  No *larv = criaNo();
  ...
  if (tk.tipo == cte::VIRGULA) {
    ...
    if (tk.tipo == cte::IDENTIFICADOR) {
      ...
      //        !existeNomeEmEscopo(smbs_var,escopo,tk.info))
      ...
      } else // funcao ==1 -> significa que é um Param, é chamado por
             // nome_função
          if (!existeNomeEmEscopo(smbs, escopo, tk.info) &&
              !existeNomeEmEscopo(smbs_var, escopo, tk.info)) {
        smbs_var.push_back(
            new SimboloParam(tk.info, escopo, (char *)"param", qnt_parms));
      ...
      // fim semantico
      tk = getToken(fin, linha, col);
      larv->filho->prox->prox =
          lista_id(fin, tk, linha, col, funcao, qnt_parms);
      return larv;
    } else {
      erroSintatico((char *)",", larv->token.info);
...
No *variavel(std::ifstream &fin, Token &tk, int *linha, int *col, int funcao,
             int &qnt_parms);
No *variaveis(std::ifstream &fin, Token &tk, int *linha, int *col, int funcao,
              int &qnt_parms) { // 1 se vem de funcao
  No *larv = criaNo();
  ...
  if (tk.tipo == cte::IDENTIFICADOR) {
    ...
    if (tk.tipo == cte::PVIRGULA) {
      ...
      tk = getToken(fin, linha, col);
      larv->filho->prox->prox =
          variaveis(fin, tk, linha, col, funcao, qnt_parms);
      return larv;
...
No *variavel(std::ifstream &fin, Token &tk, int *linha, int *col, int funcao,
             int &qnt_parms) { // 1 se é param, vem de funcao
  ...
  if (tk.tipo == cte::IDENTIFICADOR) {
    ...
    if (tk.tipo == cte::DOISPONTOS) {
      ...
      //            setTipoArraySimbolo(smbs_var,tk.info);// colocar o tipo em
      //            smbs_var ...
      // semantico
      if (!strcmp("<IDENTIFICADOR>",
                  larv->filho->prox->prox->prox->filho->token.info)) {
        strcpy(iden, larv->filho->prox->prox->prox->filho->filho->token.info);
        if (existeNomeEmEscopo(
                smbs, (char *)"global",
                larv->filho->prox->prox->prox->filho->filho->token.info)) {
          setTipoArraySimbolo(
              smbs_var,
              larv->filho->prox->prox->prox->filho->filho->token.info);
          smbs.merge(smbs_var);
      ...
      erroSintatico((char *)":", larv->token.info);
      // std::cout<<"erro em variavel, : não encontrado.   encontrado
      // "<<tk.info<<" "<<tk.linha<<std::endl;
      free(larv);
    }
    ...
    if (tk.tipo == cte::IGUAL) {
      ...
      if (!existeNomeEmEscopo(smbs, escopo, iden)) {
        if (!strcmp("integer", larv->filho->prox->prox->filho->token.info)) {
          smbs.push_back(
              new SimboloTipo(iden, escopo, (char *)"tipo", (char *)"integer"));
        } else if (!strcmp("real",
                           larv->filho->prox->prox->filho->token.info)) {
          smbs.push_back(
              new SimboloTipo(iden, escopo, (char *)"tipo", (char *)"real"));
        } else if (!strcmp("array",
                           larv->filho->prox->prox->filho->token.info)) {
          smbs.push_back(
              new SimboloTipo(iden, escopo, (char *)"tipo", (char *)"array",
                              atoi(larv->filho->prox->prox->filho->prox->prox
                                       ->filho->token.info),
                              larv->filho->prox->prox->filho->prox->prox->prox
                                  ->prox->prox->filho->token.info));
          //                            std::cout<<"--pa- "<<
          //                            larv->filho->prox->prox->filho->prox->prox->filho->token.info
          //                            <<std::endl; std::cout<<"--pa- "<<
          //                            larv->filho->prox->prox->filho->prox->prox->prox->prox->prox->filho->token.info
          //                            <<std::endl; std::cout<<"--pa- "<<
          //                            <<std::endl; new SimboloTipo()
        } else if (!strcmp("<IDENTIFICADOR>",
                           larv->filho->prox->prox->filho->token.info)) {
          //                            std::cout<<"entrando aki"<<std::endl;
          if (!existeNomeEmEscopo(
                  smbs, (char *)"global",
                  larv->filho->prox->prox->filho->filho->token.info)) {
            strcpy(desc, escopo);
            strcat(desc, " tipo não declarado - ");
...
No *nome_funcao(std::ifstream &fin, Token &tk, int *linha, int *col) {
  ...
  if (tk.tipo == cte::IDENTIFICADOR) {
    ...
    if (!existeNomeEmEscopo(smbs, escopo, tk.info)) {
      SimboloFuncao *sf =
          new SimboloFuncao(tk.info, (char *)"global", (char *)"funcao");
      //
      ...
      if (tk.tipo == cte::APARENTE) {
        ...
        //            strcpy(escopo,iden);           // setar flag para
        //            parametro

        tk = getToken(fin, linha, col);
        larv->filho->prox->prox->prox =
            variaveis(fin, tk, linha, col, 1,
                      qnt_params); // passando 1, significa q essa variavel será
                                   // paramentro, vindo de nome_funcao
                                   // inicialmente a qnt_parms é 0
        //            int total_de_params =
        //            contarParamFuncao(larv->filho->prox->prox->prox);
        //            std::cout<<"total de parametos "<<
        //            total_de_params<<std::endl; std::cout<<"$$$$$
        //            "<<qnt_params<<std::endl;
        ...
        if (tk.tipo == cte::FPARENTE) {
          larv->filho->prox->prox->prox =
              insereLista(larv->filho->prox->prox->prox, tk);
          //                strcpy(escopo,escopo_anterior);// voltando o escopo
          //                anterior
          tk = getToken(fin, linha, col);
          return larv;
        } else {
...
No *exp_logica_2(std::ifstream &fin, Token &tk, int *linha, int *col) {
  ...
  if ((tk.tipo == cte::MAIOR) || (tk.tipo == cte::MENOR) ||
      (tk.tipo == cte::IGUAL) || (tk.tipo == cte::EXCLAMACAO)) {
...
No *exp_matematica_2(std::ifstream &fin, Token &tk, int *linha, int *col) {
  ...
  if ((tk.tipo == cte::MAIS) || (tk.tipo == cte::MENOS) ||
      (tk.tipo == cte::ASTERISCO) || (tk.tipo == cte::BARRA)) {
...
No *comandos(std::ifstream &fin, Token &tk, int *linha, int *col) {
  ...
  if ((tk.tipo == cte::IDENTIFICADOR) || (tk.tipo == cte::ENQUANTO) ||
      (tk.tipo == cte::SE) || (tk.tipo == cte::ESCREVER) ||
      (tk.tipo == cte::LER)) {
...
int erroExpLogica(No *larv) {
  ...
  if (!strcmp(larv->filho->filho->token.info, "<NOME>")) {
    ...
    if (!existeNomeEmEscopo(smbs, escopo,
                            larv->filho->filho->filho->filho->token.info) &&
        !existeNomeEmEscopo(smbs, (char *)"global",
                            larv->filho->filho->filho->filho->token
                                .info)) { // verificação sementico
      ...
      //            std::cout<<larv->filho->prox->prox->filho->token.info<<std::endl;
      //            // erro pq acessa posição da memória que não "tem"
      //            std::cout<<larv->filho->filho->filho->filho->token.tipo<<std::endl;
      char *temp = tipoNomeEmEscopo(
          smbs, escopo, larv->filho->filho->filho->filho->token.info);
      if (!strcmp(temp, "")) // se o cara é igual a "", ele n esta nesse escopo,
                             // vamos pro outro
        temp = tipoNomeEmEscopo(smbs, (char *)"global",
                                larv->filho->filho->filho->filho->token.info);
...
void verificafuncao(char *nomefunc, No *larv, int &params, int totalparams) {
  ...
  if (larv->filho->filho != NULL &&
      !strcmp(larv->filho->filho->token.info, "<NOME>")) {
    tipo = tipoNomeEmEscopo(smbs, escopo,
                            larv->filho->filho->filho->filho->token.info);
    //        std::cout<<larv->filho->filho->filho->filho->token.info;
    //        std::cout<<nomefunc<<" "<<tipo<< " " <<
    //        tipoParamFuncao(smbs,nomefunc,params) <<" " <<
    //        totalparams<<std::endl;

    if (!strcmp(tipo, tipoParamFuncao(smbs, nomefunc, params))) {
      if (larv->filho->prox->filho != NULL) {
        //                std::cout<<"*****"<<std::endl;
        //                std::cout<< "**** "<<
        //                larv->filho->prox->filho->token.info<<std::endl;
        //                std::cout<< "### "<<
        //                larv->filho->prox->filho->prox->token.info<<std::endl;

        if (params + 1 < totalparams) {
          params++;
          verificafuncao(nomefunc, larv->filho->prox->filho->prox, params,
                         totalparams);
        } else {
          //                    std::cout<< " tem mais parametros que o
          //                    declarado"<<std::endl;
          ...
          strcat(desc,
                 "\" possui mais parametros de entrada do que foi declarada");
          erroSemantico(larv->token.info, desc);
        }
      }
  ...
  if (larv->filho->filho != NULL &&
      !strcmp(larv->filho->filho->token.info, "<NUMERO>")) {
...
void verificaAtribuicao(No *larv) { // semantico
  ...
  if (!strcmp(larv->filho->token.info, "<IDENTIFICADOR>")) {
    if (existeNomeEmEscopo(smbs, escopo, larv->filho->filho->token.info) ||
        existeNomeEmEscopo(smbs, (char *)"global",
                           larv->filho->filho->token.info)) {

      char *classe;
      classe = classeNomeEmEscopo(smbs, (char *)"global",
                                  larv->filho->filho->token.info);
      if (!strcmp(classe, "funcao")) { // aki passo a arvore de parametro
        //            int params_atual=0;
        int params = 0;
        int qnttotalparams =
            qntParamsFuncao(smbs, larv->filho->filho->token.info);
        verificafuncao(larv->filho->filho->token.info,
                       larv->filho->prox->filho->prox, params, qnttotalparams);

        if (params + 1 <
            qnttotalparams) { // erro mais parametros de entrada esperados
          strcpy(desc, escopo);
          std::stringstream ss;
          ss << params;
          ...
          strcat(desc, "\" possui menos parametros de entrada do que foi "
                       "declarada. linha ");
          strcat(desc, ss.str().c_str());
          erroSemantico(larv->token.info, desc);
        } else if (larv->filho->prox->filho->prox->filho->filho == NULL &&
                   qnttotalparams > 0) {
          std::stringstream ss;
          ss << params;
          ss << larv->filho->filho->token.linha;
  ...
  } else if (!strcmp(larv->filho->token.info, "NUMERO")) {
  }
}
No *comando(std::ifstream &fin, Token &tk, int *linha, int *col) {
  No *larv = criaNo();
  ...
  if (tk.tipo == cte::ENQUANTO) {
    ...
    if (!erroExpLogica(larv->filho->prox)) { // semantico -> não tem erro
      larv->filho->prox->prox = bloco(
          fin, tk, linha, col); // só ativar depois q funcinar as expressoes
      return larv;
  ...
  } else if (tk.tipo == cte::ESCREVER) {
    larv->filho = insereLista(larv->filho, tk);
    larv->filho->prox = const_valor(fin, tk, linha, col);
    tk = getToken(fin, linha, col);
    return larv;
  } else if (tk.tipo == cte::LER) {
    larv->filho = insereLista(larv->filho, tk);
    tk = getToken(fin, linha, col);
    ...
    larv->filho = nome(fin, tk, linha, col);
    //        std::cout<< "--- "<< larv->filho->filho->filho->token.info<<"-- "
    //        <<escopo<<std::endl;
    strncpy(iden, larv->filho->filho->filho->token.info, 21);
    if (existeNomeEmEscopo(smbs, escopo,
                           larv->filho->filho->filho->token.info) ||
        existeNomeEmEscopo(
            smbs, (char *)"global",
            larv->filho->filho->filho->token.info)) { // verificação sementico
      if (strcmp(classeNomeEmEscopo(smbs, escopo,
                                    larv->filho->filho->filho->token.info),
                 "const")) {
...
No *funcoes(std::ifstream &fin, Token &tk, int *linha, int *col) {
  ...
  if (tk.tipo == cte::INTEIRO || tk.tipo == cte::REAL ||
      tk.tipo == cte::VETOR || tk.tipo == cte::IDENTIFICADOR) {
...
No *funcao(std::ifstream &fin, Token &tk, int *linha, int *col) {
  ...
  //    std::cout<< "teste----- tipo função --" <<
  //    larv->filho->filho->filho->token.info << std::endl; std::cout<< "teste
  //    ---- nome função "<<larv->filho->filho->prox->filho->token.info<<
  //    std::endl; std::cout<< "teste ---- param função
  //    "<<larv->filho->filho->prox->prox->prox->filho->filho->prox->prox->filho->token.info<<
  //    std::endl;
...
No *def_var(std::ifstream &fin, Token &tk, int *linha, int *col) {
  ...
  if (tk.tipo == cte::VARIAVEL) {
    ...
    if (tk.tipo == cte::IDENTIFICADOR) {
      larv->filho->prox =
          variavel(fin, tk, linha, col, 0,
                   qnt_params); // passando 0, pq 0 significa que variavel não
                                // vem de uma declaração de função
      if (tk.tipo == cte::PVIRGULA) {
        larv->filho->prox->prox = insereLista(larv->filho->prox->prox, tk);
        tk = getToken(fin, linha, col);
        larv->filho->prox->prox->prox =
            variaveis(fin, tk, linha, col, 0,
                      qnt_params); // passando 0, pq 0 significa que variavel
                                   // não vem de uma declaração de função
                                   //
        return larv;
      } else {
        erroSintatico((char *)";", larv->token.info);
...
No *declaracoes(std::ifstream &fin, Token &tk, int *linha,
                int *col) { // inicializa as funções
...
No *corpo(std::ifstream &fin, Token &tk, int *linha, int *col) {
  ...
  if (tk.tipo == cte::TIPO || tk.tipo == cte::CONSTANTE ||
      tk.tipo == cte::VARIAVEL || tk.tipo == cte::FUNCAO) {
...
No *identificador(Token tk) {
  if (tk.tipo == cte::IDENTIFICADOR) {
    ...
    //        if(!procuraSimbolo(smbs,tk.info))
    //        smbs.push_back(Simbolo(tk.info)); // colocando na "talela" de
    //        simbolos
    larv->filho = insereLista(larv->filho, tk);
    return larv;
  } else {
...
int main(int argc, char **argv) {
  ...
  //        std::cout << tk.info << " " << tk.linha << " " << tk.col << " "<<
  //        tk.tipo <<std::endl;
  //    }
  smbs.push_back(new Simbolo((char *)"result", (char *)"global", (char *)"var",
                             (char *)"real"));
